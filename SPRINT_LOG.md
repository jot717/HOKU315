...
* **PHASE1-G PROJECT GOVERNANCE & PRODUCT CLARITY v1**: `docs/active/product/PRODUCT_MASTER.md`, `REPO_GOVERNANCE_RULES.md`, `backlog/MASTER_BACKLOG.md`, `docs/active/uat/UAT_MASTER_GUIDE.md`; README indexes; `docs/deprecated/archive/` + `backlog/archive/`; home guest/account CTAs; login/nav clarity.
...
* 建立 **AI-NATIVE REPO LAYERING v1**（[`ops/governance/REPO_ARCHITECTURE.md`](ops/governance/REPO_ARCHITECTURE.md)、`product/`、`ops/`、`ai/replay/`、`ai/incident/README.md`；`scripts`/`tests`/`docs` 仍於 repo 根目錄以相容 pytest／CI）

* **INTERFACE STABILIZATION v1**：[`ops/governance/ARCHITECTURE_CONTRACT.md`](ops/governance/ARCHITECTURE_CONTRACT.md)、`product/INTERFACE.md`、`ai/INTERFACE.md`、`ops/INTERFACE.md`
