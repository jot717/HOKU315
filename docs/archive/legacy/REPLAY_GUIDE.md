...
1. **Observe**：使用者／監控觀察到異常（見 `ops/debug/DEBUG_GUIDE.md`）。  
...
5. **Live replay（未來）**：在隔離環境或 SQL Editor 依 `rpc.sql`／`network.json` 重播；須遵守 `ops/debug/DEBUG_POLICY.md`（禁止未佐證直接改 production SQL）。
