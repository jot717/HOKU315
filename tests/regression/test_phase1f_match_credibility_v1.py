...
def test_match_archetype_docs_exist() -> None:
    arch = (ROOT / "docs/archive/phase1_legacy/MATCH_ARCHETYPE_SYSTEM.md").read_text(encoding="utf-8")
    assert "stable_low_pressure" in arch
    assert "asynchronous_safe" in arch
    energy = (ROOT / "docs/archive/phase1_legacy/SOCIAL_ENERGY_MODEL.md").read_text(encoding="utf-8")
    assert "response debt" in energy
    assert (ROOT / "docs/archive/phase1_legacy/MATCH_CREDIBILITY_CONSTITUTION.md").is_file()
    assert (ROOT / "ops/product/MATCH_SYSTEM.md").is_file()
    assert (ROOT / "docs/archive/old_uat/PHASE1F_MATCH_CREDIBILITY_UAT.md").is_file()
